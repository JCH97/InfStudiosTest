...
import { PersistEntity } from 'apps/shared/domain/entity.abstract';
import { has } from 'lodash';
import { Types } from 'mongoose';
import { IRepository } from '../interfaces/IRepository';
import {
  getDefaultPaginatedFindResult,
  PaginatedFindResult,
} from '../paginated-find.result';
...
export class BaseRepository<T extends PersistEntity> implements IRepository<T> {
  ...
  async paginatedFind(
    ...
    // const nWhere: any = await this.setIsActiveCondition(where);
  ...
  private async setIsActiveCondition(where: any): Promise<any> {
    if (has(where, '$and')) {
      where[`$and`].push({
        deleted: false,
      });
      ...
      const newWhere: any = { $and: [{ deleted: false }] };
      Object.keys(where).forEach((e) => {
        newWhere[e] = where[e];
      });
      return newWhere;
